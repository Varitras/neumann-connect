...
class SSCClient:
    ...
    async def get_many(self, paths: list[tuple[str, ...]]) -> dict:
        """Fragt mehrere Wurzeläste in einer Anfrage ab (effizienter als einzeln)."""
        combined: dict[str, Any] = {}
        for path in paths:
            _deep_merge(combined, _build_nested(path, None))
        return await self.request(combined)
    ...
    def extract(data: dict, path: tuple[str, ...]) -> Any:
        """Öffentlicher Zugriff auf _extract, für die Auswertung von get_many()-Ergebnissen."""

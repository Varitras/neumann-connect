...
### Hinzugefügt
- Geräte-Simulator (`tools/ssc_simulator.py`), der das verifizierte
  Firmware-Verhalten von KH 120 II und KH 750 nachbildet, sodass die
  Integration ohne Hardware vollständig getestet werden kann
- End-to-End-Tests, die die Integration gegen den Simulator einrichten, sowie
  Tests für den Simulator selbst (insgesamt 42 Tests)

### Geändert
- Meldungen an den Nutzer folgen der Home-Assistant-Sprache statt fest deutsch
  zu sein: Fehlermeldungen laufen über Übersetzungsschlüssel, und
  Benachrichtigungen, dynamische Labels im Einrichtungsdialog, EQ-Container-
  Namen sowie der Zustand „nicht zugewiesen" liegen auf Deutsch und Englisch vor
- Repo-Sprache ist durchgängig Englisch (Kommentare, Docstrings, Log- und
  Entwicklermeldungen); die zweisprachige Dokumentation bleibt bestehen
- Testsuite setzt Linux (WSL2 oder CI) und Home Assistant 2026.7 voraus. Die
  langsamen End-to-End-Tests sind standardmäßig abgewählt, damit der
  Alltagslauf schnell bleibt; die kontinuierliche Integration fährt immer alle

### Entfernt
- Drei ungenutzte Speicher-Module aus einem früheren Umbau
